...
function tsunami_update!(dx_buffer::AbstractMatrix{T},
                         ...
                         state::AbstractArray{T,3},
                         model_matrices::NSWE.Matrices{T},
                         params::ModelParameters) where T

    tsunami_update!(dx_buffer, dy_buffer, xnonlinear_buffer, ynonlinear_buffer, state, params.n_integration_step,
                    params.dx, params.dy, params.time_step, model_matrices)

end

# Update tsunami wavefield with NSWE in-place.
function tsunami_update!(dx_buffer::AbstractMatrix{T},
                         dy_buffer::AbstractMatrix{T},
                         xnonlinear_buffer::AbstractMatrix{T},
                         ynonlinear_buffer::AbstractMatrix{T},
                         ...
                         dy::Real,
                         time_interval::Real,
                         model_matrices::NSWE.Matrices{T}) where T
    ...
    for it in 1:nt
        ...
        NSWE.timestep!(dx_buffer, dy_buffer, xnonlinear_buffer, ynonlinear_buffer, eta_f, mm_f, nn_f, eta_a, mm_a, nn_a, model_matrices, dx, dy, dt)
    end
...
function init_arrays(nx::Int, ny::Int, n_state_var::Int, nobs::Int, nprt_per_rank::Int)
    ...
    field_buffer = Array{T}(undef, nx, ny, 4, nthreads())

    return StateVectors(state_particles, state_truth), ObsVectors(obs_truth, obs_model), StationVectors(ist, jst), field_buffer
end

function set_initial_state!(states::StateVectors, model_matrices::NSWE.Matrices{T},
                            field_buffer::AbstractArray{T, 4},
                            rng::AbstractVector{<:Random.AbstractRNG},
                            nprt_per_rank::Int,
                            params::ModelParameters) where T
    ...
    NSWE.initheight!(@view(states.truth[:, :, 1]), model_matrices, params.dx, params.dy,
...
function set_stations!(ist::AbstractVector, jst::AbstractVector, filename::String, distance_x::T, distance_y::T, boundary_x::T, boundary_y::T, dx::T, dy::T) where T
    ...
    if filename != ""
        ...
        NSWE.set_stations!(ist,jst,distance_x,distance_y,boundary_x,boundary_y,dx,dy)
    end
...
function ParticleDA.update_truth!(d::ModelData, _)
    tsunami_update!(@view(d.field_buffer[:, :, 1, 1]),
                    ...
                    @view(d.field_buffer[:, :, 3, 1]),
                    @view(d.field_buffer[:, :, 4, 1]),
...
function ParticleDA.update_particle_dynamics!(d::ModelData, nprt_per_rank)
    ...
    Threads.@threads for ip in 1:nprt_per_rank
        tsunami_update!(@view(d.field_buffer[:, :, 1, threadid()]), @view(d.field_buffer[:, :, 2, threadid()]),
                        @view(d.field_buffer[:, :, 3, threadid()]), @view(d.field_buffer[:, :, 4, threadid()]),
