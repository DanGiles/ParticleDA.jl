...
function timestep!(dx_buffer::AbstractMatrix{T},
                   ...
                   xnonlinear_buffer::AbstractMatrix{T},
                   ynonlinear_buffer::AbstractMatrix{T},
    ...
    for j in 1:ny
        for i in 2:nx
            ...
            @inbounds xnonlinear_buffer[i,j] = ((mm0[i, j]^2/x_averaged_depth[i, j]) - (mm0[i-1, j]^2/x_averaged_depth[i-1, j]))/dx
            @inbounds ynonlinear_buffer[i,j] = ((mm0[i, j]*nn0[i, j]/x_averaged_depth[i, j]) - (mm0[i-1, j]*nn0[i-1, j]/y_averaged_depth[i-1, j]))/dx
        end
        @inbounds dx_buffer[1,j] = (eta0[1,j] - 0) / dx
        @inbounds xnonlinear_buffer[1,j] = (mm0[1, j]^2/x_averaged_depth[1, j])/dx
        @inbounds ynonlinear_buffer[1,j] = (mm0[1, j]*nn0[1, j]/x_averaged_depth[1, j])/dx
    end
    for i in 1:nx
        for j in 2:ny
            @inbounds dy_buffer[i,j] = (eta0[i,j] - eta0[i, j - 1]) / dy
            @inbounds xnonlinear_buffer[i,j] += ((mm0[i, j]*nn0[i, j]/y_averaged_depth[i, j]) - (mm0[i, j-1]*nn0[i, j-1]/y_averaged_depth[i, j-1]))/dy
            @inbounds ynonlinear_buffer[i,j] += ((nn0[i, j]^2/y_averaged_depth[i, j]) - (nn0[i, j-1]^2/y_averaged_depth[i, j-1]))/dy
        end
        @inbounds dy_buffer[i,1] = (eta0[i,1] - 0) / dy
        @inbounds xnonlinear_buffer[i,1] = (mm0[i, 1]*nn0[i, 1]/y_averaged_depth[i, 1])/dy
        @inbounds ynonlinear_buffer[i,1] = (mm0[i, 1]^2/y_averaged_depth[i, 1])/dy
    end

    # Update Velocity
    for j in 1:ny, i in 1:nx
        @inbounds mm1[i,j] = mm0[i, j] - (g_n * x_averaged_depth[i, j] * dx_buffer[i, j] + xnonlinear_buffer[i,j]) * dt
        @inbounds nn1[i,j] = nn0[i, j] - (g_n * y_averaged_depth[i, j] * dy_buffer[i, j] + ynonlinear_buffer[i,j]) * dt
    end
...
function timestep!(dx_buffer::AbstractMatrix{T},
    ...
    return timestep!(dx_buffer, dy_buffer, xnonlinear_buffer, ynonlinear_buffer, eta1, mm1, nn1, eta0, mm0, nn0,
